...
describe('Intcode program', () => {
	test('Opcode 1 adds together numbers', () => {
		expect(
			computeSequence([1, 9, 10, 3, 99, 3, 11, 0, 99, 30, 40, 50]),
		).toEqual([1, 9, 10, 70, 99, 3, 11, 0, 99, 30, 40, 50])
	})
	test('Opcode 2 multiplies together numbers', () => {
		expect(
			computeSequence([1, 9, 10, 70, 2, 3, 11, 0, 99, 30, 40, 50], 4),
	...
	test('Unknown opcode should throw an error', () => {
		expect(() =>
			computeSequence([3500, 9, 10, 70, 2, 3, 11, 0, 99, 30, 40, 50]),
		).toThrow(/Unknown opcode 3500/)
	})
	test.each([
		[
			[1, 0, 0, 0, 99],
