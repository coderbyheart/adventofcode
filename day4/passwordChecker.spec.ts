...
describe('Password Checker', () => {
	it.each([
		...
		[223450, false],
		[123789, false],
	])('should mark %p as %p', (password, valid) =>
		expect(checkPassword(password as number)).toEqual(valid),
	)
})
...
describe('Password Checker with larger group checking', () => {
	it.each([
		...
		expect(checkPasswordWithLargerGroup(password as number)).toEqual(valid),
	)
})
